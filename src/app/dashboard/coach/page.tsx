      ...
      <div className="content">
        ...
        <p className="results-count">
          {searching ? (lang === 'fr' ? 'Recherche...' : 'Searching...') :
            players.length === 0 ? T.coach_no_match :
            `${T.coach_showing} ${players.length} ${players.length !== 1 ? T.coach_players : T.coach_player}${hasFilters ? ` ${T.coach_filtered}` : ''}`
          }
        </p>
              ...
              {players.map(player => {
                ...
                return (
                  <a key={player.id} href={`/cv/${player.share_token}`} className="player-row" target="_blank" rel="noopener noreferrer">
                    ...
                    <span className="row-cv-btn">{T.coach_view_cv_short}</span>
                  </a>
                )
              })}
            </div>
