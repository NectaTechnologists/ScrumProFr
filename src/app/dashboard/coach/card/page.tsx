          ...
          {shareToken && (
            <div style={{ padding: '12px 20px', background: 'rgba(212,168,67,0.06)', borderTop: '1px solid rgba(212,168,67,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                ...
                <a href={cardUrl} target="_blank" style={{ background: '#D4A843', color: '#0C0F16', border: 'none', padding: '6px 14px', borderRadius: '6px', fontSize: '11px', textDecoration: 'none', fontFamily: 'Arial, sans-serif', whiteSpace: 'nowrap', fontWeight: 700 }}>
                  Share
                </a>
              </div>
            </div>
          ...
          <>
            <div className="cp-section">
              ...
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">FULL NAME</label><input className="cp-input" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="e.g. Mike Henderson" /></div>
                <div className="cp-field"><label className="cp-label">ROLE / TITLE</label><input className="cp-input" value={form.role_title} onChange={e => setForm(f => ({ ...f, role_title: e.target.value }))} placeholder="e.g. Head Coach" /></div>
              </div>
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">CURRENT ORGANISATION</label><input className="cp-input" value={form.organisation} onChange={e => setForm(f => ({ ...f, organisation: e.target.value }))} placeholder="e.g. Bristol Bears" /></div>
                <div className="cp-field"><label className="cp-label">ORG TYPE</label><select className="cp-input" value={form.org_type} onChange={e => setForm(f => ({ ...f, org_type: e.target.value }))}><option value="">Select type</option>{ORG_TYPES.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
              </div>
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">COUNTRY</label><input className="cp-input" value={form.country} onChange={e => setForm(f => ({ ...f, country: e.target.value }))} placeholder="e.g. United Kingdom" /></div>
                <div className="cp-field"><label className="cp-label">HEADSHOT URL</label><input className="cp-input" value={form.headshot_url} onChange={e => setForm(f => ({ ...f, headshot_url: e.target.value }))} placeholder="https://..." /></div>
              </div>
            </div>
            <div className="cp-section">
              <div className="cp-section-title">BIO & PHILOSOPHY</div>
              <div className="cp-row" style={{ marginBottom: '16px' }}><div className="cp-field"><label className="cp-label">BIO</label><textarea className="cp-input cp-textarea" value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="A short professional bio..." style={{ minHeight: '100px' }} /></div></div>
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">COACHING STYLE</label><input className="cp-input" value={form.coaching_style} onChange={e => setForm(f => ({ ...f, coaching_style: e.target.value }))} placeholder="e.g. High-intensity, possession-based" /></div>
                <div className="cp-field"><label className="cp-label">SPECIALTIES</label><input className="cp-input" value={form.specialties} onChange={e => setForm(f => ({ ...f, specialties: e.target.value }))} placeholder="e.g. Set-piece, Defence, Forwards" /></div>
              </div>
              <div className="cp-row" style={{ marginBottom: '0' }}><div className="cp-field"><label className="cp-label">COACHING PHILOSOPHY</label><textarea className="cp-input cp-textarea" value={form.philosophy} onChange={e => setForm(f => ({ ...f, philosophy: e.target.value }))} placeholder="Describe your core coaching beliefs..." /></div></div>
            </div>
            <div className="cp-section">
              <div className="cp-section-title">LINKS</div>
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">LINKEDIN URL</label><input className="cp-input" value={form.linkedin_url} onChange={e => setForm(f => ({ ...f, linkedin_url: e.target.value }))} placeholder="https://linkedin.com/in/..." /></div>
                <div className="cp-field"><label className="cp-label">HIGHLIGHT VIDEO URL</label><input className="cp-input" value={form.video_url} onChange={e => setForm(f => ({ ...f, video_url: e.target.value }))} placeholder="https://youtube.com/..." /></div>
              </div>
            </div>
        ...
        {tab === 'credentials' && (
          <>
            <div className="cp-section">
              ...
              {credentials.length === 0 && <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)', marginBottom: '16px' }}>No credentials added yet.</p>}
              {credentials.map((c, i) => (
                <div key={i} className="entry-card">
                  ...
                  <div className="cp-row cp-row-2" style={{ marginBottom: '12px' }}>
                    <div className="cp-field"><label className="cp-label">CREDENTIAL / LICENCE</label><input className="cp-input" value={c.credential_type} onChange={e => updateCredential(i, 'credential_type', e.target.value)} placeholder="e.g. World Rugby Level 2" /></div>
                    <div className="cp-field"><label className="cp-label">ISSUING BODY</label><input className="cp-input" value={c.issuer} onChange={e => updateCredential(i, 'issuer', e.target.value)} placeholder="e.g. World Rugby" /></div>
                  </div>
                  <div className="cp-row cp-row-2" style={{ marginBottom: '0' }}>
                    <div className="cp-field"><label className="cp-label">YEAR AWARDED</label><input className="cp-input" type="number" value={c.year} onChange={e => updateCredential(i, 'year', e.target.value)} placeholder="e.g. 2018" min="1980" max="2030" /></div>
                    <div className="cp-field"><label className="cp-label">NOTES (optional)</label><input className="cp-input" value={c.notes} onChange={e => updateCredential(i, 'notes', e.target.value)} placeholder="e.g. Merit distinction" /></div>
                  </div>
                </div>
              ))}
        ...
        {tab === 'history' && (
          <>
            <div className="cp-section">
              ...
              {history.length === 0 && <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)', marginBottom: '16px' }}>No history added yet.</p>}
              {history.map((h, i) => (
                <div key={i} className="entry-card">
                  <button className="entry-remove" onClick={() => removeHistory(i)}>×</button>
                  <div className="cp-row cp-row-2" style={{ marginBottom: '12px' }}>
                    <div className="cp-field"><label className="cp-label">CLUB / TEAM</label><input className="cp-input" value={h.club} onChange={e => updateHistory(i, 'club', e.target.value)} placeholder="e.g. Leinster Rugby" /></div>
                    <div className="cp-field"><label className="cp-label">ROLE / TITLE</label><input className="cp-input" value={h.role} onChange={e => updateHistory(i, 'role', e.target.value)} placeholder="e.g. Defence Coach" /></div>
                  </div>
                  <div className="cp-row cp-row-3" style={{ marginBottom: '12px' }}>
                    <div className="cp-field"><label className="cp-label">FROM</label><input className="cp-input" type="number" value={h.start_year} onChange={e => updateHistory(i, 'start_year', e.target.value)} placeholder="2018" min="1970" max="2030" /></div>
                    <div className="cp-field"><label className="cp-label">TO (blank if current)</label><input className="cp-input" type="number" value={h.end_year} onChange={e => updateHistory(i, 'end_year', e.target.value)} placeholder="2023" min="1970" max="2030" /></div>
                    <div className="cp-field"><label className="cp-label">LEVEL</label><select className="cp-input" value={h.level} onChange={e => updateHistory(i, 'level', e.target.value)}><option value="">Select</option>{LEVELS.map(l => <option key={l} value={l}>{l}</option>)}</select></div>
                  </div>
                  <div className="cp-field" style={{ marginBottom: '0' }}>
                    <label className="cp-label">KEY ACHIEVEMENTS</label>
                    <textarea className="cp-input cp-textarea" value={h.key_achievements} onChange={e => updateHistory(i, 'key_achievements', e.target.value)} placeholder="e.g. Led team to European cup final..." style={{ minHeight: '68px' }} />
                  </div>
                </div>
              ))}
        ...
        {tab === 'playing' && (
          <>
            <div className="cp-section">
              ...
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">POSITION PLAYED</label><select className="cp-input" value={form.played_position} onChange={e => setForm(f => ({ ...f, played_position: e.target.value }))}><option value="">Select position</option>{POSITIONS.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                <div className="cp-field"><label className="cp-label">HIGHEST LEVEL</label><select className="cp-input" value={form.played_level} onChange={e => setForm(f => ({ ...f, played_level: e.target.value }))}><option value="">Select level</option>{LEVELS.map(l => <option key={l} value={l}>{l}</option>)}</select></div>
              </div>
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">MAIN CLUB(S)</label><input className="cp-input" value={form.played_club} onChange={e => setForm(f => ({ ...f, played_club: e.target.value }))} placeholder="e.g. Bath RFC, London Wasps" /></div>
                <div className="cp-field"><label className="cp-label">PLAYING ERA</label><input className="cp-input" value={form.played_era} onChange={e => setForm(f => ({ ...f, played_era: e.target.value }))} placeholder="e.g. 1998–2011" /></div>
              </div>
              <div className="cp-row cp-row-2">
                <div className="cp-field"><label className="cp-label">NATIONALITY (AS PLAYER)</label><input className="cp-input" value={form.played_nationality} onChange={e => setForm(f => ({ ...f, played_nationality: e.target.value }))} placeholder="e.g. Irish" /></div>
              </div>
            </div>
            <div className="cp-save-wrap">
